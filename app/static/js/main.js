...
!(function (a, b) {
  ...
  (p.alert = function () {
    ...
    (p.prompt = function () {
      ...
      if (
        (q.setAttribute("type", a.inputType),
        o[a.inputType] && (a.inputType = q.type),
        (a.message = f),
        (a.buttons.cancel.callback = a.onEscape =
          function () {
      ...
      switch (((h = b(n.inputs[a.inputType])), a.inputType)) {
        ...
        case "select":
          ...
          g(k, function (a, d) {
            ...
            d.group &&
              (r[d.group] ||
                (r[d.group] = b("<optgroup/>").attr("label", d.group)),
              (e = r[d.group])),
              e.append(
    ...
    (p.dialog = function (a) {
      ...
      if (
        ...
        d.find(".bootbox-body").html(a.message),
        ...
        a.title
          ? c.find(".modal-header").prepend(k)
          : k.css("margin-top", "-10px").prependTo(d);
      }
      return (
        a.title && c.find(".modal-title").html(a.title),
        i.length && (d.after(n.footer), c.find(".modal-footer").html(i)),
...
var Apppages = {
  ...
  formid: "",
  formsubmit: function () {
    consolelog("Load Apppages.formsubmit");
    $("#" + Apppages.formid)
      .unbind("on")
      .on("submit", function (event) {
        ...
        if ($(this).attr("method") == "post") {
          ...
          var options = {
            beforeSubmit: Apppages.showRequest,
            success: Apppages.showResponse,
            url: urlaction,
          };
  ...
  showRequest: function (formData, jqForm, options) {
    ...
    for (var i = 0; i < formData.length; i++) {
      if (formData[i].type == "textarea") {
        consolelog(formData[i].value);
        if (formData[i].value == "<p><br></p>") {
          consolelog("Textarea vacio");
          formData[i].value = "";
        }
      }
    }
  ...
  showResponse: function (responseText, statusText, xhr, form) {
    ...
    Apppages.validateForm(responseText, form);
  },
    ...
    $.each($.parseJSON(data), function (idx, obj) {
      ...
      if (obj.redirect != "") {
        ...
        if (obj.message_error != "") {
          removeLoadScreen();
          $("#" + idform + " #responseForm").show();
          $("#" + idform + " #responseForm")
            .html(
              '<div class="alert alert-warning">' + obj.message_error + "</div>"
            )
            .fadeIn(1500)
            .delay(5000)
            .fadeOut(1000);
        ...
        if (obj.message_success != "") {
          ...
          Obj = window[Apppages.actualmodel];
          invokeMethod(Obj, "beforeSubmit", data);
        }
      }
        ...
        $.each(obj.errors, function (idmodel, objerrors) {
          ...
          $.each(modelerrors, function (idname, objerror) {
            if (objerror[0]) {
              ...
              $("input[name*='data[" + model + "][" + idname + "]']").after(
                '<div class="error alert alert-danger">' + errorinput + "</div>"
              );
              $("select[name*='data[" + model + "][" + idname + "]']").after(
                '<div class="error alert alert-danger">' + errorinput + "</div>"
              );
              $("textarea[name*='data[" + model + "][" + idname + "]']").after(
                '<div class="error alert alert-danger">' + errorinput + "</div>"
              );
              if (idname == "recaptcha") {
                $(".g-recaptcha").after(
              ...
              $.each(objerror, function (locale, errormsg) {
                $(
                  ...
                  '<div class="error alert alert-danger">' + errormsg + "</div>"
                );
                $(
                  "select[name*='data[" +
                    model +
  ...
  clickBlockScreen: function () {
    consolelog("Load Apppages.clickBlockScreen");
    $("a").bind("click", function (event) {
      consolelog($(this));
      if (
        $(this).hasClass("deleteitem") === false &&
...
var newsletters = {
  footer: function () {
    consolelog("Load newsletters.footer");
    Apppages.actualmodel = "newsletters";
    Apppages.formid = "NewsletterIndexForm";
    Apppages.formsubmit();
  },
  formdetails: function () {
    consolelog("Load newsletters.formdetails");
  },
  beforeSubmit: function (data) {
    consolelog("Load newsletters.beforeSubmit");
  },
};
var checkout = {
  paymentoptionsdata: {},
  loadtypeinfo: function (id) {
    if (isEmpty(checkout.paymentoptionsdata) == false) {
      divextrainfo = $("#moreinformationpayment");
      if (
        typeof checkout.paymentoptionsdata.data.paymentextrainfo != "undefined"
      ) {
        if (
          typeof checkout.paymentoptionsdata.data.paymentextrainfo[id] !=
          "undefined"
        ) {
          extrainfo = checkout.paymentoptionsdata.data.paymentextrainfo[id];
          divextrainfo.html("");
          divextrainfo.html(extrainfo).show();
        } else {
          divextrainfo.html("").hide();
        }
      }
    }
  },
  showdatarequest: function () {
    showdatarequest = $(
      'input[type=radio][name="data[Order][paymentoption_id]"]:checked'
    ).attr("data-request-field");
    if (showdatarequest == 1) {
      labelrequest = $(
        'input[type=radio][name="data[Order][paymentoption_id]"]:checked'
      ).attr("data-request-label");
      $("#data_request").show();
      $("#data_request").find("input[type=text]").removeAttr("disabled");
      $("#data_request").find("input[type=text]").val("");
      $("#data_request").find("label").html(labelrequest);
    } else {
      $("#data_request").find("input[type=text]").val("");
      $("#data_request").find("input[type=text]").attr("disabled", "disabled");
      $("#data_request").hide();
    }
  },
  changetypepayment: function () {
    $('input[type=radio][name="data[Order][paymentoption_id]"]').change(
      function () {
        loadingScreen();
        type = $(
          'input[type=radio][name="data[Order][paymentoption_id]"]:checked'
        ).val();
        checkout.loadtypeinfo(type);
        checkout.showdatarequest();
        removeLoadScreen();
      }
    );
    type = $(
  ...
  paymentoptions: function () {
    $("#data_request").hide();
    var order_subtotal = $("#subtotal_amount").html().trim();
    var order_discount = 0;
    if ($("#discount_mount").length) {
      if ($("#discount_mount").html().trim() != "-0") {
        order_discount = $("#discount_mount").html().trim().slice(2);
      }
    }
    var order_total = $("#total_mount").html().trim();
    $(".typepayment").each(function () {
      type = $(this);
      type.find(".totalamount").html(checkout.formatNumber(order_total));
      console.log("Total: " + order_total);
      order_total = order_total.replace(",", "");
      order_subtotal = order_subtotal.replace(",", "");
      console.log("Subtotal " + order_subtotal);
      order_currency = order_total.charAt(0);
      order_subtotal_mount =
        parseFloat(order_subtotal.slice(1)) - parseFloat(order_discount);
      order_total_mount = parseFloat(order_total.slice(1));
      processingdata = type.find(".processingamount");
      p_type = $(processingdata).attr("data-pf");
      p_sign = processingdata.attr("data-sign");
      p_porcentage = processingdata.attr("data-porcentage");
      p_amount = parseFloat(
        order_subtotal_mount * (p_porcentage / 100)
      ).toFixed(2);
      processingdata.html(p_sign + order_currency + p_amount);
      console.log("Amount: " + p_amount);
      console.log(order_total_mount);
      if (p_sign == "+") {
        order_grand_total = (
          parseFloat(order_total_mount) + parseFloat(p_amount)
        ).toFixed(2);
      } else {
        order_grand_total = (
          parseFloat(order_total_mount) - parseFloat(p_amount)
      ...
      grandtotal = type
        .find(".gtotalmount")
        .html(order_currency + checkout.formatNumber(order_grand_total));
      type.attr("data-total", order_grand_total);
    });
    var $wrapper = $("#paymentsOptions");
    $wrapper
      .find(".typepayment")
      .sort(function (a, b) {
      ...
      .appendTo($wrapper);
    $(".typepayment:last").removeClass("normal").addClass("reddanger");
    $(".typepayment:first").removeClass("normal").addClass("featured");
    $(".typepayment:first").find(".radio").attr("checked", "checked");
    checkout.changetypepayment();
  },
  formatNumber: function (num) {
    return num.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
  },
  neworder: function () {
    consolelog("Load checkout");
    Apppages.actualmodel = "Order";
    Apppages.formid = "OrderIndexForm";
    Apppages.formsubmit();
    $("#OrderEmail").focusout(function () {
      ...
      if (!$(this).is("[readonly]")) {
        ...
        $.ajax({
          method: "GET",
          dataType: "json",
          url: "/checkout?a=checkemail&email=" + email,
          ...
          if (data.error != "") {
            ...
            idname = "email";
            $("#OrderEmail").after(
              '<div class="error alert alert-danger">' + data.error + "</div>"
            );
          } else {
            checkout.paymentoptionsdata = data;
            if (data.data["order_c"] > 0) {
              $("#OrderEmailconfirmation").val(email);
              $("#OrderFullname").val(data.data["fullname"]);
              $("#OrderPhone").val(data.data["phone"]);
              $("#OrderAddress").val(data.data["address"]);
              $("#OrderCity").val(data.data["city"]);
              $("#OrderState").val(data.data["state"]);
              $("#OrderZipcode").val(data.data["zipcode"]);
              $("#OrderEmail").attr("readonly", "readonly");
              $("#OrderEmailconfirmation").attr("readonly", "readonly");
            } else {
              $("#OrderEmailconfirmation").removeAttr("readonly");
            }
            if (data.data["paymentoptions"] != "") {
              $("#paymentoptions").html(data.data["paymentoptions"]);
              $("#paymentoptions").show();
              checkout.paymentoptions();
            }
            $("#OrderFullname").removeAttr("readonly");
            $("#OrderPhone").removeAttr("readonly");
            $("#OrderAddress").removeAttr("readonly");
            $("#OrderCity").removeAttr("readonly");
            $("#OrderState").removeAttr("readonly");
            $("#OrderZipcode").removeAttr("readonly");
    ...
    $("input#OrderCity,input#OrderState,input#OrderZipcode").keyup(function () {
      var val = $(this).val();
      $(this).val(val.toUpperCase());
    });
    $("textarea#OrderAddress").keyup(function () {
      var val = $(this).val();
      $(this).val(val.toUpperCase());
    });
    $("#applycupon").on("click", function (e) {
      ...
      loadingScreen();
      $("#msg_coupon").html("");
      coupon_code = $("#OrderCouponCode").val();
      $.post("/checkout_coupons", { coupon_code: coupon_code }).done(function (
        data
      ) {
        $.each($.parseJSON(data), function (idx, obj) {
          if (obj.message_error == "") {
            ...
            totals = dataobj.totals;
            $("#discount_code").html(totals.coupon_code);
            $("#discount_mount").html(
              "-" + currency.symbol + "" + totals.coupon_discount
            );
            $("#total_mount").html(currency.symbol + "" + totals.total);
            $("#OrderCouponCode").attr("readonly", "readonly");
            $("#applycupon").hide();
            $("#discount_block").show();
            checkout.paymentoptions();
          } else {
            $("#msg_coupon").html(obj.message_error);
          }
        });
        removeLoadScreen();
      });
  ...
  formdetails: function () {
    consolelog("Load checkout.formdetails");
  },
  beforeSubmit: function (data) {
    consolelog("Load checkout.beforeSubmit");
  },
};
var contact = {
  sendcontact: function () {
    consolelog("Load contact.footer");
    Apppages.actualmodel = "contact";
    Apppages.formid = "ContactpageIndexForm";
    Apppages.formsubmit();
  },
  formdetails: function () {
    consolelog("Load contact.formdetails");
  },
  beforeSubmit: function (data) {
    consolelog("Load contact.beforeSubmit");
  },
};
var testimonial = {
  sendtestimonial: function () {
    consolelog("Load testimonial.footer");
    Apppages.actualmodel = "testimonial";
    Apppages.formid = "TestimonialpageIndexForm";
    Apppages.formsubmit();
  },
  formdetails: function () {
    consolelog("Load contact.formdetails");
...
LiveHelpSettings.server = "www.easybuypharma.com";
...
function modelCheckorder() {
  ...
  $("#formcheckorder").on("submit", function (e) {
    e.preventDefault();
    orderid = $("#fco_orderid").val();
    email = $("#fco_email").val();
    newtarget = $(this).attr("action") + orderid + "/" + email;
...
function chat() {
  ...
  (function (d, $, undefined) {
    $(window).ready(function () {
      var LiveHelp = d.createElement("script");
      LiveHelp.type = "text/javascript";
      LiveHelp.async = true;
      LiveHelp.src =
        ("https:" == d.location.protocol ? "https://" : "http://") +
        LiveHelpSettings.server +
        "/livehelp/scripts/jquery.livehelp.min.js";
      var s = d.getElementsByTagName("script")[0];
      s.parentNode.insertBefore(LiveHelp, s);
    });
  })(document, jQuery);
}
function changeTypeShipping() {
  $("#shippingtype").val($(".typeShipping:checked").val());
  $(".typeShipping")
    .unbind("change")
...
function calculeTotal() {
  priceshipping = $(".typeShipping:checked").attr("data-price");
  total = $(".finalAmount").attr("data-total");
  currency = $(".finalAmount").attr("data-currency");
  if (total) {
    if (priceshipping) {
      totalshipping = (parseFloat(total) + parseFloat(priceshipping)).toFixed(
        2
      );
    } else {
      totalshipping = parseFloat(total).toFixed(2);
    }
    $(".finalAmount").html(currency + totalshipping);
  }
}
...
function shippingCart() {
  ...
  $("#OrderCountry").change(function () {
    ...
    if (country != "") {
      $.ajax({
        method: "GET",
        url: "/cart?a=shippingTypes&country=" + country,
        beforeSend: function (xhr) {
          $("#shippingTypes").removeClass("show");
          $("#shippingTypes").empty();
          $("#shippingTypes").hide();
          $("#shippingtype").val("");
          calculeTotal();
        ...
        $("#shippingTypes").html(data);
        $("#shippingTypes").show();
        changeTypeShipping();
        calculeTotal();
      });
    } else {
      $(".typeShipping:checked").attr("checked", false);
      calculeTotal();
      $("#shippingtype").val("");
      $("#shippingTypes").empty();
      $("#shippingTypes").hide();
      ...
      if (shippingtype == "") {
        ...
        $.post(
          $(this).attr("href"),
          {
            ...
            shippingcountry: $("#OrderCountry").val(),
          },
          function (data) {
            console.log(data);
...
function modalOrder() {
  if ($("#Modalreadmore").length > 0) {
    $("#Modalreadmore").foundation("reveal", "open");
    $("#Modalreadmore input.input-submit").attr("disabled", "disabled");
    current_button = $("#Modalreadmore input.input-submit").val();
    var counter = 5;
    $("#Modalreadmore input.input-submit").val("Wait " + counter + " seconds");
    var interval = setInterval(function () {
      counter--;
      $("#Modalreadmore input.input-submit").val(
        "Wait " + counter + " seconds"
      );
      if (counter == 0) {
        $("#Modalreadmore input.input-submit").removeAttr("disabled");
        $("#Modalreadmore input.input-submit").val(current_button);
        clearInterval(interval);
      }
    }, 1000);
    $("#Modalreadmore input.input-submit").on("click", function () {
      $("#Modalreadmore").foundation("reveal", "close");
    });
  }
}
  ...
  (function (d, $, undefined) {
    $(window).ready(function () {
      ...
      LiveHelp.src =
        ("https:" == d.location.protocol ? "https://" : "http://") +
        LiveHelpSettings.server +
        "/livehelp/scripts/jquery.livehelp.js";
      var s = d.getElementsByTagName("script")[0];
      s.parentNode.insertBefore(LiveHelp, s);
    });
  })(document, jQuery);
